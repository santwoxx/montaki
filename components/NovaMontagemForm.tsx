...
export function NovaMontagemForm({
  ...
  // Compartilhado entre a importação por OCR/XML e as notas pendentes de
  // sistemas externos: seleciona a loja resolvida/recém-cadastrada e garante
  // que ela apareça na lista mesmo que tenha acabado de ser criada agora.
    ...
    <form action={action} className="space-y-6">
      ...
      <div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Valor da assistência (R$) (opcional)">
            <div>
              <Input
                type="text"
                inputMode="decimal"
                name="valorAssistencia"
                defaultValue={valoresIniciais?.valorAssistencia ?? ""}
                placeholder="Ex: 6,00"
              />
              <p className="mt-1 text-xs text-slate-500">Apenas se houver assistência nessa nota. Esse valor é recebido integralmente pela empresa.</p>
            </div>
          </Field>
          <Field
