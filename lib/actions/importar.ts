...
async function importarXml(buffer: Buffer): Promise<DadosImportados> {
  const parser = new XMLParser({ ignoreAttributes: false, parseTagValue: false });
  const doc: unknown = parser.parse(buffer.toString("utf-8"));
...
export async function importarNotaAction(formData: FormData): Promise<DadosImportados> {
  ...
  try {
    if (nomeArquivo.endsWith(".xml") || arquivo.type.includes("xml")) {
      return await importarXml(buffer);
    }
    return {
      erro:
...
function interpretarTextoNota(textoOriginal: string): DadosImportados {
  ...
  function cortarEmOutroCampo(valor: string | undefined): string | undefined {
    if (!valor) return undefined;
    const m = valor.match(RE_OUTRO_CAMPO);
    const cortado = (m && m.index !== undefined ? valor.slice(0, m.index) : valor).trim();
  ...
  const clienteNome = valorAposRotulo(
    linhas,
  ...
  const numeroPedido = primeiroMatch(texto, /N[ºo°]\s*[:.]?\s*(\d{1,9})\b/);
...
export async function importarNotaTextoAction(texto: string): Promise<DadosImportados> {
  ...
  const limpo = String(texto || "").slice(0, 20000);
  if (limpo.trim().length < 15) {
    return {
      erro: "Não consegui reconhecer texto suficiente nessa imagem. Tente uma foto mais nítida e bem enquadrada.",
    };
  }
